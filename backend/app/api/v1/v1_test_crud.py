...
def test_user_register(create: CreateUser):
    ...
    new_user = crud_test.create_user(create)
    if new_user:
...
def test_update_userinfo(put: UpdateUser, current_user=Depends(get_current_user)):
    ...
    if current_user.email == put.email:
        ...
        try:
            ...
            raise HTTPException(status_code=403, detail='邮箱格式错误，请重新输入')
    crud_test.update(current_user, put)
