...
async def create_dept(obj: DeptCreate):
    check = await DeptDao.get_one_dept_by_name(obj.name)
    if check:
        return Response403(msg='部门已存在')
    else:
        data = await DeptDao.create_dept(obj)
        return Response200(data=data)


@dept.put('/{id}', summary='修改部门', dependencies=[Depends(rbac.verify_rbac)])
async def update_dept(obj: DeptUpdate, id: int = Query(...)):
    check = await DeptDao.get_one_dept_by_id(id)
    if not check:
        return Response404(data=obj)
    check_name = await DeptDao.get_one_dept_by_name(obj.name)
    if obj.name != check.name:
        if check_name:
            return Response403(msg='部门已存在, 请更换部门名称')
    data = await DeptDao.update_dept(id, obj)
    return Response200(data=data)
...
async def delete_dept(id: int = Query(...)):
    ...
    if not check:
        return Response404()
    if await DeptDao.delete_dept(id):
        return Response200()
    return Response500()
