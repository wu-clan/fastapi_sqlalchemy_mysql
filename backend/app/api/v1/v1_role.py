...
async def create_depm(obj: RoleCreate, db: AsyncSession = Depends(get_db)):
    check = await crud_role.get_one_role_by_name(db, obj.name)
    if check:
        return Response403(msg='角色已存在')
    else:
        data = await crud_role.create_role(db, obj)
...
async def create_depm(obj: RoleUpdate, id: int = Query(...), db: AsyncSession = Depends(get_db)):
    check = await crud_role.get_one_role_by_id(db, id)
    if not check:
        return Response404(data=obj)
    check_name = await crud_role.get_one_role_by_name(db, obj.name)
    if obj.name != check.name:
        if check_name:
            return Response403(msg='角色已存在, 请更换角色名称')
    data = await crud_role.update_role(db, id, obj)
    return Response200(data=data)
...
async def get_depm(id: int = Query(...), db: AsyncSession = Depends(get_db)):
    ...
    if not check:
        return Response404()
    if await crud_role.delete_role(db, id):
        return Response200()
    return Response500()
