...
async def get_tree_menus():
    data_list = await crud_menu.get_all_menus()
...
async def delete_menu(id: int = Query(...)):
    ...
    if not check:
        return Response404()
    await crud_menu.delete_menu(id)
    return Response200()
