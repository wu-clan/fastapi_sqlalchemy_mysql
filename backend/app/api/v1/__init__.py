...
v1.include_router(depm, prefix='/depm', tags=['部门管理'], dependencies=[Depends(rbac.verify_rbac)])
v1.include_router(api, prefix='/api', tags=['API管理'], dependencies=[Depends(rbac.verify_rbac)])
v1.include_router(role, prefix='/role', tags=['角色管理'], dependencies=[Depends(rbac.verify_rbac)])
