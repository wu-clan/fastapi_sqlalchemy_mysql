...
async def user_login(form_data: OAuth2PasswordRequestForm = Depends()) -> Any:
    ...
    # 更新登陆时间
    await UserDao.update_user_login_time(form_data.username)
    # 创建token
...
async def user_login_email_captcha(request: Request, obj: ELCode) -> Any:
    if not await UserDao.check_email(obj.email):
...
async def user_login_email(request: Request, obj: Auth2) -> Any:
    ...
    # 更新登陆时间
    await UserDao.update_user_login_time(username)
    # 创建token
    access_token = create_access_token(current_user.id)
    uid = current_user.user_uid
    rd_token = await redis_client.get(uid)
...
async def user_register(obj: CreateUser) -> Any:
    ...
    email = await UserDao.check_email(obj.email)
    if email:
    ...
    new_user = await UserDao.create_user(obj)
