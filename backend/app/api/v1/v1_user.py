...
# async def user_login(user_info: Auth):
#     current_user = await crud_user.get_user_by_username(user_info.username)
#     if not current_user:
#         raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='用户名不存在', headers=headers)
...
async def get_email_login_code(request: Request, email: ELCode, tasks: BackgroundTasks):
    if not await crud_user.check_email(email.email):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='邮箱不存在', headers=headers)
    username = await crud_user.get_username_by_email(email.email)
...
async def user_login(request: Request, email: Auth2):
    username = await crud_user.get_username_by_email(email.email)
    current_user = await crud_user.get_user_by_username(username)
    if not current_user.is_active:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='该用户已被锁定，无法登录', headers=headers)
...
async def user_register(create: CreateUser, role: CreateUserRole):
    username = await crud_user.get_user_by_username(create.username)
    if username:
        raise HTTPException(status_code=403, detail='该用户名已被注册~ 换一个吧')
    email = await crud_user.check_email(create.email)
    if email:
    ...
    dept = await crud_dept.get_one_dept_by_id(create.department_id)
    if not dept:
        raise HTTPException(status_code=404, detail='所选部门不存在')
    if len(role.role_id) == 1:
        if not await crud_role.get_one_role_by_id(role.role_id):
    ...
    new_user = await crud_user.create_user(create, role)
    if new_user:
...
async def password_reset(reset_pwd: ResetPassword, request: Request, response: Response):
    ...
    if not await crud_user.reset_password(request.cookies.get('fast-username'), reset_pwd.password2):
...
async def userinfo(current_user=Depends(get_current_user)):
    if current_user:
        if await crud_user.get_user_by_id(current_user.id):
            info = await crud_user.get_userinfo(current_user.id)
            if info:
...
async def update_userinfo(
        put: UpdateUser = Depends(UpdateUser),
        role: UpdateUserRole = Depends(UpdateUserRole),
    ...
    if current_user.username == put.username:
        ...
        username = await crud_user.get_user_by_username(put.username)
        if username:
            raise HTTPException(status_code=403, detail='该用户名已存在~ 换一个吧')
    if current_user.email == put.email:
        pass
    else:
        _email = await crud_user.check_email(put.email)
        if _email:
        ...
        try:
            ...
            raise HTTPException(status_code=403, detail='邮箱格式错误，请重新输入')
    dept = await crud_dept.get_one_dept_by_id(put.department_id)
    if not dept:
        raise HTTPException(status_code=404, detail='所选部门不存在')
    if len(role.role_id) == 1:
        if not await crud_role.get_one_role_by_id(role.role_id):
    ...
    if current_user:
        await crud_user.update_userinfo(current_user, put, role, _file)
...
async def super_set(pk: int):
    if await crud_user.get_user_by_id(pk):
        if await crud_user.super_set(pk):
            return Response200(msg=f'修改超级权限成功，当前：{await crud_user.get_user_is_super(pk)}')
        return Response200(msg=f'修改超级权限成功，当前：{await crud_user.get_user_is_super(pk)}')
...
async def active_set(pk: int):
    if await crud_user.get_user_by_id(pk):
        if await crud_user.active_set(pk):
            return Response200(msg=f'修改用户状态成功, 当前：{await crud_user.get_user_is_action(pk)}')
        return Response200(msg=f'修改用户状态成功, 当前：{await crud_user.get_user_is_action(pk)}')
    return Response404(msg=f'用户 {pk} 不存在')
