...
# def user_login(user_info: Auth):
#     current_user = crud_user.get_user_by_username(user_info.username)
#     if not current_user:
#         raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='用户名不存在', headers=headers)
...
#         raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='该用户已被锁定，无法登录', headers=headers)
#     # 更新登陆时间
#     crud_user.update_user_login_time(user_info.username)
#     # 查询用户角色
#     user_roles = crud_user.get_user_roles(current_user.id)
#     # 创建token
#     access_token = create_access_token([current_user.id, user_roles])
...
# def user_login(request: Request, user_info: Auth2):
#     current_user = crud_user.get_user_by_username(user_info.username)
#     if not current_user:
#         raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='用户名不存在', headers=headers)
#     elif not jwt_security.verity_password(user_info.password, current_user.password):
...
def user_register(create: CreateUser, role: CreateUserRole):
    username = crud_user.get_user_by_username(create.username)
    if username:
        raise HTTPException(status_code=403, detail='该用户名已被注册~ 换一个吧')
    email = crud_user.check_email(create.email)
    if email:
    ...
    if len(role.role_id) == 1:
        if not crud_role.get_one_role_by_id(role.role_id[0]):
    ...
    new_user = crud_user.create_user(create, role)
    if new_user:
...
def password_reset(resetpwd: ResetPassword, request: Request, response: Response):
    ...
    try:
        crud_user.reset_password(request.cookies.get('fast-username'), resetpwd.password2)
    except Exception as e:
...
def userinfo(current_user=Depends(get_current_user)):
    if current_user:
        if crud_user.get_user_by_id(current_user.id):
            info = crud_user.get_userinfo(current_user.id)
            if info:
