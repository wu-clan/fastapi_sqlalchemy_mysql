...
def user_login(form_data: OAuth2PasswordRequestForm = Depends()) -> Any:
    ...
    # 查询用户角色
    user_roles = crud_user.get_user_roles(current_user.id)
    # 创建token
...
def user_register(obj_user: CreateUser, obj_role: CreateUserRole) -> Any:
    username = crud_user.get_user_by_username(obj_user.username)
    if username:
        raise HTTPException(status_code=403, detail='该用户名已被注册~ 换一个吧')
    email = crud_user.check_email(obj_user.email)
    if email:
    ...
    dept = crud_dept.get_one_dept_by_id(obj_user.department_id)
    if not dept:
    ...
    if len(obj_role.role_id) == 1:
        if not crud_role.get_one_role_by_id(obj_role.role_id[0]):
        ...
        for _ in obj_role.role_id:
            if not crud_role.get_one_role_by_id(_):
                raise HTTPException(status_code=404, detail=f'所选角色 {_} 不存在')
    new_user = crud_user.create_user(obj_user, obj_role)
...
def password_reset(obj: ResetPassword, request: Request, response: Response) -> Any:
    ...
    if request.cookies.get('code') != sha256(obj.code.encode('utf-8')).hexdigest():
...
def update_userinfo(
    ...
    if current_user.username == username:
        ...
        _username = crud_user.get_user_by_username(username)
        if _username:
    ...
    if len(role) < 1:
        raise HTTPException(status_code=403, detail='必须至少选择一个角色')
    if len(role[0]) < 3:
        if not crud_role.get_one_role_by_id(role[0]):
    ...
    crud_user.update_userinfo(current_user, department_id, username, email, mobile_number, wechat, qq, blog_address,
                              introduction, role, _file)
...
def get_all_users() -> Any:
    with get_db() as session:
        return paginate(session, crud_user.get_users())


@user.post('/{id}/super', summary='修改用户超级权限', dependencies=[Depends(jwt_security.get_current_is_superuser)])
def super_set(id: int) -> Any:
    if crud_user.get_user_by_id(id):
        if crud_user.super_set(id):
            return Response200(msg=f'修改超级权限成功', data=crud_user.get_user_is_super(id))
        return Response200(msg=f'修改超级权限成功', data=crud_user.get_user_is_super(id))
...
def active_set(id: int) -> Any:
    if crud_user.get_user_by_id(id):
        if crud_user.active_set(id):
            return Response200(msg=f'修改用户状态成功', data=crud_user.get_user_is_action(id))
        return Response200(msg=f'修改用户状态成功', data=crud_user.get_user_is_action(id))
    return Response404(msg='用户不存在')
