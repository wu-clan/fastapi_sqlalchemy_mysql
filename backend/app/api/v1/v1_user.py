...
def user_register(create: CreateUser, db: Session = Depends(get_db)):
    ...
    new_user = crud_user.create_user(db, create)
    if new_user:
...
def update_userinfo(put: UpdateUser = Depends(UpdateUser), file: UploadFile = File(None),
    ...
    if current_user.username == put.username:
        ...
        username = crud_user.get_user_by_username(db, put.username)
        if username:
            raise HTTPException(status_code=403, detail='该用户名已存在~ 换一个吧')
    if current_user.email == put.email:
        pass
    else:
        _email = crud_user.check_email(db, put.email)
        if _email:
    ...
    if current_user:
        crud_user.update_userinfo(db, current_user, put, _file)
