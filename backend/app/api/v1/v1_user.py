...
async def user_login(form_data: OAuth2PasswordRequestForm = Depends()) -> Any:
    ...
    # 更新登陆时间
    await UserDao.update_user_login_time(current_user)
    # 查询用户角色
    user_roles = await UserDao.get_user_roles(current_user.id)
    # 创建token
...
async def user_login_email_captcha(request: Request, obj: ELCode) -> Any:
    current_email = await UserDao.check_email(obj.email)
    if not current_email:
        raise HTTPException(status_code=404, detail='邮箱不存在', headers=headers)
    username = await UserDao.get_username_by_email(obj.email)
    current_user = await UserDao.get_user_by_username(username)
...
async def user_register(obj_user: CreateUser, obj_role: CreateUserRole) -> Any:
    username = await UserDao.get_user_by_username(obj_user.username)
    if username:
        raise HTTPException(status_code=403, detail='该用户名已被注册~ 换一个吧')
    email = await UserDao.check_email(obj_user.email)
    if email:
    ...
    dept = await DeptDao.get_one_dept_by_id(obj_user.department_id)
    if not dept:
    ...
    if len(obj_role.role_id) == 1:
        if not await RoleDao.get_role_by_id(obj_role.role_id[0]):
        ...
        for _ in obj_role.role_id:
            if not await RoleDao.get_role_by_id(_):
                raise HTTPException(status_code=404, detail=f'所选角色 {_} 不存在')
    new_user = await UserDao.create_user(obj_user, obj_role)
...
async def password_reset_captcha(username_or_email: str, response: Response) -> Any:
    code = text_captcha()
...
async def update_userinfo(
    ...
    if current_user.username == username:
        ...
        username = await UserDao.get_user_by_username(username)
        if username:
    ...
    if current_user.email == email:
        ...
        try:
            ...
            raise HTTPException(status_code=403, detail='邮箱格式错误，请重新输入')
    dept = await DeptDao.get_one_dept_by_id(department_id)
    if not dept:
        raise HTTPException(status_code=404, detail='所选部门不存在')
    if len(role) < 1:
        raise HTTPException(status_code=403, detail='必须至少选择一个角色')
    if len(role[0]) < 3:
        if not await RoleDao.get_role_by_id(role[0]):
    ...
    new_user = await UserDao.update_userinfo(current_user, department_id, username, email, mobile_number, wechat, qq,
...
async def get_all_users() -> Any:
    async with get_db() as session:
        return await paginate(session, UserDao.get_users())
...
async def super_set(id: int) -> Any:
    if await UserDao.get_user_by_id(id):
        if await UserDao.super_set(id):
            return Response200(msg=f'修改超级权限成功', data=await UserDao.get_user_is_super(id))
        return Response200(msg=f'修改超级权限成功', data=await UserDao.get_user_is_super(id))
...
async def active_set(id: int) -> Any:
    if await UserDao.get_user_by_id(id):
        if await UserDao.active_set(id):
            return Response200(msg=f'修改用户状态成功', data=await UserDao.get_user_is_action(id))
        return Response200(msg=f'修改用户状态成功', data=await UserDao.get_user_is_action(id))
    return Response404(msg=f'用户 {id} 不存在')
