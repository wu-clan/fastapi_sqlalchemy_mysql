...
def user_login(form_data: OAuth2PasswordRequestForm = Depends()) -> Any:
    current_user = UserDao.get_user_by_username(form_data.username)
    if not current_user:
    ...
    # 更新登陆时间
    UserDao.update_user_login_time(form_data.username)
    # 查询用户角色
    user_roles = UserDao.get_user_roles(current_user.id)
    # 创建token
...
#         raise HTTPException(status_code=401, detail='该用户已被锁定，无法登录', headers=headers)
#     # 更新登陆时间
#     UserDao.update_user_login_time(obj.username)
#     # 查询用户角色
#     user_roles = UserDao.get_user_roles(current_user.id)
#     # 创建token
#     access_token = create_access_token([current_user.id, user_roles])
...
def user_register(obj_user: CreateUser, obj_role: CreateUserRole) -> Any:
    username = UserDao.get_user_by_username(obj_user.username)
    if username:
        raise HTTPException(status_code=403, detail='该用户名已被注册~ 换一个吧')
    email = UserDao.check_email(obj_user.email)
    if email:
        raise HTTPException(status_code=403, detail='该邮箱已被注册~ 换一个吧')
    try:
        validate_email(obj_user.email, check_deliverability=False).email
    except EmailNotValidError:
        raise HTTPException(status_code=403, detail='邮箱格式错误，请重新输入')
    dept = DeptDao.get_one_dept_by_id(obj_user.department_id)
    if not dept:
    ...
    if len(obj_role.role_id) == 1:
        if not RoleDao.get_one_role_by_id(obj_role.role_id[0]):
            raise HTTPException(status_code=404, detail=f'所选角色 {obj_role.role_id[0]} 不存在')
    elif len(obj_role.role_id) > 1:
        for _ in obj_role.role_id:
            if not RoleDao.get_one_role_by_id(_):
                raise HTTPException(status_code=404, detail=f'所选角色 {_} 不存在')
    new_user = UserDao.create_user(obj_user, obj_role)
...
def password_reset_captcha(username_or_email: str, response: Response, tasks: BackgroundTasks) -> Any:
    code = text_captcha()
    if UserDao.get_user_by_username(username_or_email):
        try:
        ...
        current_user_email = UserDao.get_email_by_username(username_or_email)
        send_verification_code_email(current_user_email, code)
        return Response200(msg='邮件发送成功')
    else:
        try:
            validate_email(username_or_email, check_deliverability=False).email
        except EmailNotValidError:
            raise HTTPException(status_code=404, detail='用户名不存在，请重新输入')
        email_result = UserDao.check_email(username_or_email)
        if not email_result:
        ...
        try:
            ...
            response.set_cookie(key='fastapi_reset_pwd_code', value=sha256(code.encode('utf-8')).hexdigest(),
                                max_age=settings.COOKIES_MAX_AGE)
            username = UserDao.get_username_by_email(username_or_email)
...
def password_reset(obj: ResetPassword, request: Request, response: Response) -> Any:
    ...
    if not UserDao.reset_password(cookie_reset_pwd_username, obj.password2):
...
def update_userinfo(
    ...
    if current_user.username == username:
        ...
        _username = UserDao.get_user_by_username(username)
        if _username:
            raise HTTPException(status_code=403, detail='该用户名已存在~ 换一个吧')
    if current_user.email == email:
        pass
    else:
        _email = UserDao.check_email(email)
        if _email:
        ...
        try:
            ...
            raise HTTPException(status_code=403, detail='邮箱格式错误，请重新输入')
    dept = DeptDao.get_one_dept_by_id(department_id)
    if not dept:
        raise HTTPException(status_code=404, detail='所选部门不存在')
    if len(role) < 1:
        raise HTTPException(status_code=403, detail='必须至少选择一个角色')
    if len(role[0]) < 3:
        if not RoleDao.get_one_role_by_id(role[0]):
    ...
    current_filename = UserDao.get_avatar_by_username(current_user.username)
    if file is not None:
    ...
    new_user = UserDao.update_userinfo(current_user, department_id, username, email, mobile_number, wechat, qq,
                                       blog_address, introduction, role, _file_name)
...
def delete_avatar(current_user=Depends(jwt_security.get_current_user)):
    current_filename = UserDao.get_avatar_by_username(current_user.username)
    if current_filename is not None:
        try:
            os.remove(AvatarPath + current_filename)
    ...
    UserDao.delete_avatar(current_user.id)
    return Response200(msg='删除用户头像成功')


@user.get('', summary='获取所有用户', response_model=Page[GetUserInfo], response_class=ORJSONResponse,
          dependencies=[Depends(jwt_security.get_current_user)])
def get_all_users() -> Any:
    with get_db() as session:
        return paginate(session, UserDao.get_users())


@user.post('/{pk}/super', summary='修改用户超级权限', dependencies=[Depends(jwt_security.get_current_is_superuser)])
def super_set(pk: int) -> Any:
    if UserDao.get_user_by_id(pk):
        if UserDao.super_set(pk):
            return Response200(msg=f'修改超级权限成功', data=UserDao.get_user_is_super(pk))
        return Response200(msg=f'修改超级权限成功', data=UserDao.get_user_is_super(pk))
...
def active_set(pk: int) -> Any:
    if UserDao.get_user_by_id(pk):
        if UserDao.active_set(pk):
            return Response200(msg=f'修改用户状态成功', data=UserDao.get_user_is_action(pk))
        return Response200(msg=f'修改用户状态成功', data=UserDao.get_user_is_action(pk))
    return Response404(msg='用户不存在')
...
def delete_user(current_user=Depends(get_current_user)) -> Any:
    ...
    try:
        ...
        UserDao.delete_user(current_user.id)
