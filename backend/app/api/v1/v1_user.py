...
def user_register(obj: CreateUser) -> Any:
    username = UserDao.get_user_by_username(obj.username)
    if username:
        raise HTTPException(status_code=403, detail='该用户名已被注册~ 换一个吧')
    email = UserDao.check_email(obj.email)
    if email:
    ...
    new_user = UserDao.create_user(obj)
...
def password_reset_captcha(username_or_email: str, response: Response) -> Any:
    code = text_captcha()
...
def update_userinfo(
    ...
    if current_user.username != username:
        username = UserDao.get_user_by_username(username)
        if username:
    ...
    new_user = UserDao.update_userinfo(current_user, username, email, mobile_number, wechat, qq, blog_address,
...
def get_all_users() -> Any:
    return paginate(UserDao.get_users())
