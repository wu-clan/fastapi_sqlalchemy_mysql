...
class CRUDUser(CRUDBase[User, CreateUser, UpdateUser]):
    ...
    def get_user_roles(self, user_id: int) -> list:
        with self.db as session:
            user_roles = session.query(UserRole.role_id).filter(UserRole.user_id == user_id).all()
            ur_list = []
            for _ in user_roles:
                ur_list.append(_[0])
            return ur_list
    ...
    def get_userinfo(self, user_id: int) -> User:
        with self.db as session:
            return session.query(User).filter(User.id == user_id).options(joinedload(User.roles)).first()
    ...
    def create_user(self, create: CreateUser, role: CreateUserRole) -> User:
        with self.db as session:
            ...
            if len(role.role_id) == 1:
                new_user.roles = [session.query(Role).get(role.role_id)]
            else:
                role_list = []
                for _ in role.role_id:
                    role_list.append(session.query(Role).get(_))
